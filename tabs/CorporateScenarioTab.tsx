            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-grow">
             ...
             <div className="flex justify-between items-center mt-6">
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => { const blob = new Blob([`Scenario: Corporate\nCognitive Load: High`], { type: 'text/plain' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'corporate-summary.txt'; a.click(); URL.revokeObjectURL(url); }} className="px-4 py-2 text-sm font-semibold text-gray-700 bg-white rounded-md border border-gray-200 hover:bg-gray-50">Download Summary</button>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={resetSimulation} className="px-4 py-2 text-sm font-semibold text-white transition-colors duration-200 bg-cyan-600 rounded-md hover:bg-cyan-500 focus:bg-cyan-700">Run New Check-in</button>
          ...
          <div className="text-center animate-fade-in flex flex-col items-center justify-center h-full bg-white p-8 rounded-2xl shadow-lg border border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-2 mb-4 w-full">
                {IMAGES.dashboard_gallery.map((src: string, i: number) => (
                  <img key={i} src={src} alt={`Corporate ${i + 1}`} className="h-20 w-full object-cover rounded-md cursor-pointer" loading="lazy" onClick={() => { setLightboxIndex(i); setLightboxOpen(true); }} />
                ))}
            </div>
          ) : (
            <img src={IMAGES.corporate_intro} alt="Employee in a modern office" className="w-full h-48 object-cover rounded-lg mb-6" loading="lazy"/>
          )}

          {lightboxOpen && (
            <ImageLightbox images={IMAGES.dashboard_gallery || [IMAGES.corporate_intro]} initialIndex={lightboxIndex} onClose={() => setLightboxOpen(false)} />
          )}
