    ...
    <div className="p-4 animate-fade-in">
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
        ...
        <div className="bg-gray-100 h-80 rounded-md flex items-center justify-center relative overflow-hidden" style={{ backgroundImage: `url(${IMAGES.home_hero})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
          <WorldMap />
          {visiblePings.map((p) => (
            <button key={p.id} className="absolute" style={{ top: p.top, left: p.left }} onClick={() => setSelectedPing(p)}>
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="flex gap-2">
            <button onClick={() => setFilter('all')} className={`px-3 py-1 rounded-md ${filter==='all'? 'bg-cyan-500 text-white' : 'bg-white border'}`}>All</button>
            <button onClick={() => setFilter('na')} className={`px-3 py-1 rounded-md ${filter==='na'? 'bg-cyan-500 text-white' : 'bg-white border'}`}>North America</button>
            <button onClick={() => setFilter('eu')} className={`px-3 py-1 rounded-md ${filter==='eu'? 'bg-cyan-500 text-white' : 'bg-white border'}`}>Europe</button>
            <button onClick={() => setFilter('as')} className={`px-3 py-1 rounded-md ${filter==='as'? 'bg-cyan-500 text-white' : 'bg-white border'}`}>Asia</button>
            <button onClick={() => setFilter('af')} className={`px-3 py-1 rounded-md ${filter==='af'? 'bg-cyan-500 text-white' : 'bg-white border'}`}>Africa</button>
          </div>
          <div>
            <button onClick={() => setGalleryOpen(true)} className="px-3 py-2 bg-cyan-500 text-white rounded-md">View Photos</button>
          </div>
        </div>
      </div>

      {selectedPing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60" onClick={() => setSelectedPing(null)}>
          <div className="bg-white rounded-lg p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-lg font-bold">{selectedPing.name}</h3>
              <button onClick={() => setSelectedPing(null)} className="text-gray-500">✕</button>
            </div>
            <p className="text-sm text-gray-600">Screenings: <strong>{selectedPing.stats.screenings}</strong></p>
            <p className="text-sm text-gray-600">Sessions: <strong>{selectedPing.stats.sessions}</strong></p>
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => { setGalleryOpen(true); }} className="px-3 py-2 bg-cyan-500 text-white rounded-md">Open Photos</button>
              <button onClick={() => { const blob = new Blob([`Region: ${selectedPing.name}\nScreenings: ${selectedPing.stats.screenings}\nSessions: ${selectedPing.stats.sessions}`], { type: 'text/plain' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${selectedPing.id}-summary.txt`; a.click(); URL.revokeObjectURL(url); }} className="px-3 py-2 bg-white border rounded-md">Download Report</button>
            </div>
          </div>
        </div>
      )}
      ...
      {galleryOpen && (
        <ImageLightbox images={(selectedPing && selectedPing.images) || IMAGES.education_gallery || [IMAGES.home_hero]} initialIndex={0} onClose={() => setGalleryOpen(false)} />
      )}
